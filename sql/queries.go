...
var CoordinatesUpdate = "update coordinates SET g=ST_GeomFromText(?) where id=?"
var ClusterCoordinate = "select o.id, o.layerId, o.name, ST_AsGeoJSON(IF(ST_GeometryType(g) = 'POLYGON', ST_EndPoint(ST_ExteriorRing(g)), ST_Centroid(g))) as geojson from coordinates c  inner join objects o on c.id  = o.id  WHERE layerId = ? and ST_GeometryType(g) IN ('POLYGON', 'MULTIPOLYGON')"
