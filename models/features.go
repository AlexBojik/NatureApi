...
func CheckPosition(check *Check) []string {
	...
	q := "SELECT name FROM layers WHERE warning and id in (SELECT DISTINCT layerId FROM objects where id in (SELECT id FROM coordinates c WHERE ST_Intersects(ST_Buffer(Point(?, ?), 0.0001), g)))"
